...
export class CreditPurchaseComponent implements OnInit {
  ...
  itemsCRP(): FormArray {
    return this.CreditPurchaseForm.get("itemsCRP") as FormArray
  }

  addMoreItem() {
    this.itemsCRP().push(this.newItemRow());
    this.itemsIndex++;
  }
  ...
  setSupplierId(event) {
    this.CreditPurchaseForm.patchValue({supplierId: this.CreditPurchaseForm.get('supplier_id').value});
  }

  calcualtTotal() {
    const form = this.CreditPurchaseForm.value;
    let total1 = 0;

    this.itemsCRP().controls.forEach(item => {
      let amount = Number(item.get('price').value) * Number(item.get('quantity').value);
      total1 += amount;
      item.patchValue({ amount: amount })
    })

    const amount = Number(form.itemsCRP.price) * Number(form.quantity);
